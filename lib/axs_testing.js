 ...
 *
 * Generated from http://github.com/GoogleChrome/accessibility-developer-tools/tree/0ee37bdb9c66c535288004643e7ff0f186efe3cf
 *
...
axs.utils.isLargeFont = function(a) {
  var b = "bold" == a.fontWeight;
  return(a = a.fontSize.match(/(\d+)px/)) ? (a = parseInt(a[1], 10), b && 14 <= a || 18 <= a) : !1;
};
...
axs.AuditRule.specs.ariaOwnsDescendant = {name:"ariaOwnsDescendant", heading:"aria-owns should not be used if ownership is implicit in the DOM", url:"", severity:axs.constants.Severity.WARNING, relevantElementMatcher:function(a) {
  return axs.browserUtils.matchSelector(a, "[aria-owns]");
}, test:function(a) {
  for (var b = a.ownerDocument, c = a.getAttribute("aria-owns").split(/\s+/), d = 0, e = c.length;d < e;d++) {
    var f = b.getElementById(c[d]);
    if (f && a.compareDocumentPosition(f) & Node.DOCUMENT_POSITION_CONTAINED_BY) {
      return!0;
    }
  }
  return!1;
...
axs.AuditRule.specs.humanLangMissing = {name:"humanLangMissing", heading:"The web page should have the content's human language indicated in the markup", url:"", severity:axs.constants.Severity.WARNING, relevantElementMatcher:function(a) {
...
axs.AuditRule.specs.multipleAriaOwners = {name:"multipleAriaOwners", heading:"An element's ID must not be present in more that one aria-owns attribute at any time", url:"https://github.com/GoogleChrome/accessibility-developer-tools/wiki/Audit-Rules#-ax_aria_07--an-element-can-have-only-one-explicit-aria-owner", severity:axs.constants.Severity.WARNING, relevantElementMatcher:function(a) {
  return axs.browserUtils.matchSelector(a, "[aria-owns]");
}, test:function(a) {
  var b = a.ownerDocument;
  a = a.getAttribute("aria-owns").split(/\s+/);
  for (var c = 0, d = a.length;c < d;c++) {
    var e = b.getElementById(a[c]);
    if (e && 1 < axs.utils.getIdReferrers("aria-owns", e).length) {
      return!0;
    }
  }
  return!1;
}, code:"AX_ARIA_07"};
axs.AuditRule.specs.nonExistentAriaRelatedElement = {name:"nonExistentAriaRelatedElement", heading:"ARIA attributes which refer to other elements by ID should refer to elements which exist in the DOM", url:"", severity:axs.constants.Severity.SEVERE, relevantElementMatcher:function(a) {
